...
export class TronCollectService extends BaseCollectService {
  ...
  async activateAddress(tx: BaseTransactionEntity): Promise<void> {
    ...
    tronUtil.sendTrx(tx.to, activationAmount).then((txHash) => {
  ...
  private async collectTRX(
    ...
    this.tronUtil.sendTrx(this.collectAddress, Number(transferAmount)).then(async (hash) => {
      // 保存归集交易记录
  ...
  private async fundTrx(
    ...
    if (!feePrivateKey) {
      this.logger.warn('Failed to get fee wallet private key');
      return;
    }
    ...
    if (!feeWalletAddress) {
      this.logger.warn('Failed to get fee wallet address');
      return;
    }
    ...
    feeTronUtil.sendTrx(toAddress, Number(gasFee)).then((txHash) => {
      this.logger.log(
